...
// Recargar gemas y registrar la recarga
const { GemTopup } = require('../models');

router.post('/:id/topup-gems', async (req, res) => {
  try {
    ...
    await GemTopup.create({ userId, amount, method });
...
// Sumar punto por mensaje en el chat y guardar el mensaje
router.post('/:id/add-chat-point', async (req, res) => {
  try {
    const userId = req.params.id;
    const { message, streamId } = req.body;
    console.log('POST /users/:id/add-chat-point llamado para userId:', userId, 'streamId:', streamId, 'message:', message);
    const user = await User.findByPk(userId);
    if (!user) {
    ...
    if (!message || !streamId) {
      return res.status(400).json({ error: 'Faltan datos del mensaje o streamId' });
    }
    ...
    // Sumar punto solo si el mensaje se guarda correctamente
    const prevPoints = user.points;
    user.points = (user.points || 0) + 1;
    // Recalcular nivel y pointsToNextLevel
    if (user.points >= user.pointsToNextLevel) {
      user.level = (user.level || 1) + 1;
      user.points = user.points - user.pointsToNextLevel;
      user.pointsToNextLevel = 100;
      console.log(`Nivel subido! Nuevo nivel: ${user.level}`);
    }
    await user.save();
    console.log(`Punto sumado por chat. Usuario: ${user.name}, Puntos antes: ${prevPoints}, Puntos ahora: ${user.points}`);
    res.json({ success: true, points: user.points, level: user.level, pointsToNextLevel: user.pointsToNextLevel });
  } catch (err) {
    console.error('Error en /users/:id/add-chat-point:', err);
    res.status(400).json({ error: err.message });
  }
});

// Obtener cantidad de streams realizados por usuario
router.get('/:id/streams/count', async (req, res) => {
  try {
    const userId = req.params.id;
    const { Stream } = require('../models');
    const count = await Stream.count({ where: { streamerId: userId } });
    res.json({ count });
  } catch (err) {
    res.status(400).json({ error: err.message });
  }
...
router.get('/:id', async (req, res) => {
  try {
    const user = await User.findByPk(req.params.id);
    if (!user) return res.status(404).json({ error: 'Usuario no encontrado' });
    // Aseguramos que HorasTotales se envía correctamente
    res.json({
      ...user.toJSON(),
      HorasTotales: user.HorasTotales
    });
  } catch (err) {
    res.status(400).json({ error: err.message });
  }
